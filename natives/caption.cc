...
Napi::Value Caption(const Napi::CallbackInfo &info) {
  ...
  try {
    ...
    string fontfile = obj.Has("fontfile") ? obj.Get("fontfile").As<Napi::String>().Utf8Value() : "Futura";
    string type = obj.Get("type").As<Napi::String>().Utf8Value();
    bool inverttoggle = obj.Has("invert") ? obj.Get("invert").As<Napi::Boolean>().Value() : false;
    int delay =
